...
if(isset($_GET['id_dmt'])){
	...
	$totalRows = mysql_num_rows(mysql_query("SELECT * FROM sp WHERE id_dmt = $id_dmt")) ;
...
elseif(isset($_GET['id_dmn'])){
	...
	$totalRows = mysql_num_rows(mysql_query("SELECT * FROM sp WHERE id_dmn = $id_dmn")) ;
	$totalPages = ceil($totalRows/$rowsPerPage);
	...
	$totalRows = mysql_num_rows(mysql_query("SELECT * FROM sp")) ;
	$totalPages = ceil($totalRows/$rowsPerPage);
	...
	<tbody>
		<?php while ($rows = mysql_fetch_array($query)) {
			...
			<tr>
				...
				<td style="text-align:center;"><img src="../Frontend/image/Source/<?php echo $rows['anh_sp']; ?>" class="img"></td>
