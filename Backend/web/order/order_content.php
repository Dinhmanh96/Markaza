...
if (isset($_POST['submit_day'])) {
	...
	$totalRows = mysql_num_rows(mysql_query("SELECT * FROM orderlist WHERE date_order <= '$day_new'  AND pay='Chưa đặt cọc'")) ;
}else{
	$sql = "SELECT * FROM orderlist ORDER BY id_order DESC LIMIT $perRow, $rowsPerPage";
	$totalRows = mysql_num_rows(mysql_query("SELECT * FROM orderlist")) ;
}
		...
		<?php while ($rows = mysql_fetch_array($query)) {
			...
			if($rows['confirm'] == 'Awaiting confirmation'){
				...
				<td class="text-center"><?php if($rows['pay']=="Chưa đặt cọc"){
					...
					// echo $rows['confirm'];
					echo ' <img style="padding-bottom: 10px" src ="../Frontend/image/yes.png" with ="5px" height ="auto">';
				}?></td>
