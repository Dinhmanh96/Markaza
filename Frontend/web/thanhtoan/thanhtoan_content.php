    ...
    if(isset($_POST['submit'])){
		...
		if(isset($ten) && isset($mail) && isset($dt) && isset($dc)){
			...
			<body>
						...
						<p>Truy cập link: <a href="http://localhost/Project/Frontend/index.php?page_layout=confirm&code_order='.$code_order.'">http://localhost/Project/Frontend/index.php?page_layout=confirm&code_order='.$code_order.'</a> để xác nhận đơn hàng</p>';
			...
			if(!$mailer->Send()){
				...
				foreach($_SESSION['giohang'] as $id_sp=>$sl){
					...
					$query1 = mysql_query($sqladdorder);

					// Lấy số lượng hiện tại của sản phẩm
					$sqlsl = "SELECT * FROM sp WHERE id_sp = $id_sp";
					$querysl = mysql_query($sqlsl);
					$rowsl = mysql_fetch_array($querysl);
					$rowsl1 = $rowsl['sl_sp'];
